...
$sql3 = "INSERT INTO `match_winner`(`tour_id`, `team_id`, `round_position`, `round_number`) VALUES ($tour_id,'$w','','1')";

$sql3u = "UPDATE `match_winner` 
SET `team_id`= $w,
`round_number`= '1' WHERE tour_id = '$tour_id'";

$sql4 = "INSERT INTO `tournament`(`teamA_id`, `teamA_score`, `teamB_id`, `teamb_score`, `round_position`, `round_number`, `round_id`, `tour_position`) 
VALUES ('$w','','','','1st','1','$z','final')";

$sql4u = "UPDATE `tournament` 
SET `teamA_id`= $w
WHERE tour_position = 'final'";

$sql6 = "INSERT INTO `tournament`(`teamA_id`, `teamA_score`, `teamB_id`, `teamb_score`, `round_position`, `round_number`, `round_id`, `tour_position`) 
VALUES ('$e','','','','3rd','3','$f','3rd')";

$sql6u = "UPDATE `tournament` 
SET `teamA_id`= '$e'
WHERE tour_position = '3rd'";


$sql5 = "UPDATE tournament 
        SET teamB_id = '$w' 
        ORDER BY tour_id DESC LIMIT 2";
...
if ($team_winnerA == 99 && $team_winnerB == 99) {
    ...
    if ($row9["teamA_score"] === '' || $row9["teamA_score"] === NULL) {
        ...
        if ($row10["teamB_id"] == '' || $row10["teamB_id"] == NULL) {
            ...
            if (
                $conn->query($sql2) && $conn->query($sql3) && $conn->query($sql5) && $conn->query($sql7)
            ) {
            ...
            if (
                $conn->query($sql2) && $conn->query($sql3) && $conn->query($sql4) && $conn->query($sql6)
            ) {
        ...
        if ($mod != 0) {
            ...
            if (
                $conn->query($sql2) && $conn->query($sql3u) && $conn->query($sql4u) && $conn->query($sql6u)
            ) {
            ...
            if (
                $conn->query($sql2) && $conn->query($sql3u) && $conn->query($sql5u) && $conn->query($sql7u)
            ) {
